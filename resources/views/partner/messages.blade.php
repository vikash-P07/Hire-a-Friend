...
@section('title', 'Messages | Hire-a-Friend Partner')
...
<div class="page-header">
    ...
    <p class="page-subtitle">Chat with your customers</p>
</div>
...
<div class="chat-layout">
    ...
    <div class="chat-sidebar" id="chatSidebar">
        ...
        <div id="newChatList" style="display:none;">
            <div class="px-3 py-2 text-muted fw-bold" style="font-size:0.75rem;background:var(--surface-2);">BOOKED CUSTOMERS</div>
            @foreach($bookedCustomers as $bc)
            <div class="chat-list-item new-chat-item" onclick="startNewChat({{ $bc->customer->id }}, '{{ addslashes($bc->customer->name) }}', '{{ $bc->customer->profile_picture_url }}')">
                <div class="position-relative">
                    @if($bc->customer->profile_picture)
                        <img src="{{ $bc->customer->profile_picture_url }}" class="chat-avatar" alt="">
                    @else
                        <div class="chat-avatar-placeholder">{{ strtoupper(substr($bc->customer->name,0,1)) }}</div>
                    @endif
                </div>
                <div class="flex-grow-1 min-width-0">
                    <div class="chat-name">{{ $bc->customer->name }}</div>
