...
@section('title', 'Recommended Profiles Management - Admin Console')
...
<div class="mb-4">
    ...
    <p class="text-muted mb-0">Select, order, and toggle visibility of profiles featured in the "Recommended for You" section on the homepage.</p>
</div>

<div class="row g-4">
    <!-- Left Column: Curated Recommended Profiles -->
    <div class="col-12 col-xl-7">
        <div class="portal-card h-100">
            <h5 class="fw-bold text-theme-primary mb-3"><i class="bi bi-star-fill text-warning me-2"></i>Current Recommended Profiles</h5>
                            ...
                            <tr data-id="{{ $p->id }}" class="align-middle">
                                ...
                                <td class="text-end">
                                    <div class="d-inline-flex gap-1">
                                        <form action="{{ route('admin.homepage.recommended.toggle-visibility', $p->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            <button type="submit" class="btn btn-sm {{ $p->is_recommended_visible ? 'btn-outline-secondary' : 'btn-success' }} rounded-pill" style="font-size: 0.75rem;">
                                                {{ $p->is_recommended_visible ? 'Disable' : 'Enable' }}
                                            </button>
                                        </form>
                                        <form action="{{ route('admin.homepage.recommended.remove', $p->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Remove from Recommended list?');">
                                            @csrf
                        ...
                        @empty
                            <tr id="empty-row">
                                <td colspan="7" class="text-center py-5 text-muted">
                                    <i class="bi bi-star d-block fs-1 mb-2"></i>
                                    No recommended profiles chosen yet. Add companions from the panel on the right.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <div class="col-12 col-xl-5">
        <div class="portal-card">
            ...
            <!-- Filters -->
            <form action="{{ route('admin.homepage.recommended') }}" method="GET" class="row g-2 mb-4">
                <div class="col-7">
                ...
                <div class="col-12 d-flex gap-2">
                    ...
                    <a href="{{ route('admin.homepage.recommended') }}" class="btn btn-sm btn-outline-secondary px-3 rounded-pill">Reset</a>
                </div>
            </form>
                    ...
                    <tbody>
                        @forelse($companions as $c)
                            <tr>
                                ...
                                <td>
                                    @php
                                        $sub = $c->activeSubscription;
                                    @endphp
                                    @if($sub && $sub->plan)
                                        <span class="badge bg-primary-subtle text-primary">{{ $sub->plan->name }}</span>
                                    @else
                                        <span class="badge bg-secondary-subtle text-secondary">Free</span>
                                    @endif
                                </td>
                                <td class="text-end">
                                    <form action="{{ route('admin.homepage.recommended.add') }}" method="POST">
                                        @csrf
        ...
        if (listEl && listEl.querySelector('tr[data-id]')) {
            new Sortable(listEl, {
                ...
                onEnd: function () {
                    ...
                    fetch('{{ route("admin.homepage.recommended.reorder") }}', {
                        method: 'POST',
                        headers: {
                    ...
                    .catch(error => {
                        console.error('Error reordering recommended profiles:', error);
                    });
                }
            });
        }
